...
public class AiService {
    ...
    @Value("${gemini.api.key:}")   // safe even if empty, but NOT used now
    private String apiKey;

    private static final String OLLAMA_URL = "http://localhost:11434/api/generate";
    private static final String OLLAMA_MODEL = "gemma3:1b"; // make sure you pulled this model
...
""";
    ...
    public String generateImprovedTip(AnalysisRequest request, AnalysisResponse response) {
        ...
        try {
            ResponseEntity<Map> apiResponse =
                    restTemplate.exchange(OLLAMA_URL, HttpMethod.POST, entity, Map.class);

            if (apiResponse.getStatusCode().is2xxSuccessful() && apiResponse.getBody() != null) {
                Object respObj = apiResponse.getBody().get("response");
                if (respObj != null) {
            ...
            System.out.println("Error calling Ollama API: " + e.getMessage());
        }
    ...
    public void enrichWithInsights(AnalysisRequest request, AnalysisResponse response) {
        ...
        System.out.println(">> AiService.enrichWithInsights CALLED (Ollama)");
        ...
        try {
            ...
            if (!apiResponse.getStatusCode().is2xxSuccessful() || apiResponse.getBody() == null) {
                System.out.println("Ollama insights: non-2xx " + apiResponse.getStatusCode());
                return;
            }

            Object respObj = apiResponse.getBody().get("response");
            if (respObj == null) {
                System.out.println("Ollama insights: no 'response' field");
                return;
            }

            String rawText = respObj.toString().trim();
            System.out.println("Ollama insights raw response: " + rawText);

            int firstBrace = rawText.indexOf('{');
            int lastBrace = rawText.lastIndexOf('}');

            if (firstBrace == -1 || lastBrace == -1 || firstBrace >= lastBrace) {
                System.out.println("Ollama insights: no JSON object found in text.");
                return;
            }
            ...
            // NEW: Remove any sentences that incorrectly talk about missing skills as strengths
            sanitizeInsights(response);


            // Optional: if everything is still empty, provide a simple deterministic fallback
            if ((response.getStrengths() == null || response.getStrengths().isEmpty())
                ...
                System.out.println("Ollama insights: all lists empty, using simple fallback.");
            ...
            System.out.println("Ollama insights generation failed: " + e.getMessage());
        }
    }

    // -------------------------
    // CAREER CHAT (WITH RAG, Ollama)
    // -------------------------
    ...
    public String answerCareerQuestion(String question, String resumeText, String jobDescription) {
        ...
        System.out.println(">> AiService.answerCareerQuestion CALLED (Ollama)");
        ...
        try {
            ...
            if (!apiResponse.getStatusCode().is2xxSuccessful() || apiResponse.getBody() == null) {
                System.out.println("Ollama Q&A failed: non-2xx " + apiResponse.getStatusCode());
            ...
            System.out.println("Error calling Ollama API for Q&A: " + e.getMessage());
        }
    ...
    // -------------------------
    // SKILL PARSING (Ollama)
    // -------------------------

    public AiSkillProfile analyzeSkillsWithAi(String jobDescription, String resumeText) {

        System.out.println(">> AiService.analyzeSkillsWithAi CALLED (Ollama)");
        ...
        try {
            ...
            if (!apiResponse.getStatusCode().is2xxSuccessful()
                ...
                System.out.println("Ollama skill analysis failed: non-2xx " + apiResponse.getStatusCode());
                return null; // let caller fallback
            }

            Object respObj = apiResponse.getBody().get("response");
            if (respObj == null) {
                System.out.println("Ollama skill analysis: no 'response' field");
                return null;
            }

            String rawText = respObj.toString();
            ...
            if (firstBrace == -1 || lastBrace == -1 || firstBrace >= lastBrace) {
                System.out.println("Ollama skill analysis: no JSON object found in text: " + cleaned);
                return null; // let caller fallback
            }
            ...
            System.out.println(">> AiService (Ollama): parsed AI profile successfully.");
