...
public class MatchService {
    ...
    // Extract skills from free text based on SKILLS list
    // Ignores weak mentions like "Introduction to Python", "Python certification"
    private List<String> extractSkills(String text) {
        ...
        for (String skill : SKILLS) {
            ...
            int skillIndex = lower.indexOf(skill);
            if (skillIndex == -1) {
                continue; // this skill doesn't appear at all
            }

            boolean weakMention = false;

            // 1) Exact phrase check (e.g., "introduction to python", "python certification")
            for (String weakKey : WEAK_SKILL_HINTS) {
                if (lower.contains(weakKey + " " + skill) ||
                        lower.contains(skill + " " + weakKey)) {
                    weakMention = true;
                    break;
                }
            }

            // 2) Proximity check: skill appears near a weak phrase
            if (!weakMention) {
                for (String weakKey : WEAK_SKILL_HINTS) {
                    int wkIndex = lower.indexOf(weakKey);
                    if (wkIndex != -1) {
                        int distance = Math.abs(wkIndex - skillIndex);
                        // e.g. "CERTIFICATION ... Python Programming" → close in text
                        if (distance < 80) {
                            weakMention = true;
                            break;
                        }
